...
export class LeaderSuggestionsService {
  ...
  private async getPerformanceMetrics(leaders: User[]): Promise<PerformanceMetrics> {
    try {
      ...
      const allLeaderStrategies = await Promise.all(leaders.map(async leader => {
      ...
      const metrics = leaders.map((leader, index) => {
        ...
        if (strategies.length > 0) {
          return {
            winRate: strategies.reduce((sum, s) => {
            ...
            monthlyReturn: strategies.reduce((sum, s) => {
      ...
      return {
        winRates: metrics
          .map(m => m.winRate)
          .filter(rate => rate >= 0),
        monthlyReturns: metrics
          .map(m => m.monthlyReturn)
  ...
  private async calculateRiskScore(
    ...
    try {
      ...
      if (strategies.length === 0 && leader.performance) {
        ...
        const winRatePercentile = this.getPercentileRank(leader.performance.winRate, metrics.winRates);
        const tradeCountPercentile = this.getPercentileRank(leader.performance.totalTrades, metrics.tradeCounts);
      ...
      const strategyRiskScore = strategies.reduce((sum, strategy) => {
        if (!strategy.riskLevel) {
          console.warn(`Missing risk level for strategy ${strategy.id}`);
          return sum + 0.5; // default to medium risk
        }
        const baseRisk = this.riskToleranceMap[strategy.riskLevel as keyof typeof this.riskToleranceMap] || 0.5;
        return sum + baseRisk;
  ...
  private async calculateStyleScore(
    ...
    try {
      ...
      if (strategies.length === 0 && leader.performance) {
        ...
        const monthlyReturnPercentile = this.getPercentileRank(leader.performance.monthlyReturn, metrics.monthlyReturns);
        const winRatePercentile = this.getPercentileRank(leader.performance.winRate, metrics.winRates);
      ...
      const strategyStyleScore = strategies.reduce((sum, strategy) => {
      ...
      if (leader.performance) {
        ...
        const finalStyle = (strategyWeight * strategyStyleScore) + (performanceWeight * performanceStyle);
  ...
  private async calculateMarketScore(
    ...
    try {
      ...
      const intersection = new Set(
        [...copierMarkets].filter(market => leaderMarkets.has(market))
      );
      
      const baseScore = intersection.size / (copierMarkets.size + leaderMarkets.size - intersection.size);
  ...
  private async calculateFrequencyScore(
    ...
    try {
      ...
      const strategyFrequencyScore = strategies.reduce((sum, strategy) => {
        if (!strategy.tradeType) {
          console.warn(`Missing trade type for strategy ${strategy.id}`);
      ...
      if (leader.performance) {
        ...
        const finalFrequency = (strategyWeight * strategyFrequencyScore) + (actualWeight * tradeCountPercentile);
  ...
  public async findMatchingLeaders(copierId: string): Promise<LeaderSuggestion[]> {
    ...
    const suggestions = await Promise.all(leaders.map(async leader => {
      const scores = {
